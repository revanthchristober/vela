 ...
 *
 * Image paths point at `/brand/*` assets generated by
 * `scripts/generate-placeholders.mjs` (run via `pnpm placeholders`) — a
 * sharp/SVG pipeline that renders a token-coloured block at the correct
 * aspect ratio, so layout, `sizes` and CLS behaviour are all real long
